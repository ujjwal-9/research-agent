"""Enhanced semantic ingestion pipeline for large documents orchestrating all components."""
...
class EnhancedSemanticIngestionPipeline:
    """
    Enhanced semantic ingestion pipeline for large documents.

    Orchestrates the entire enhanced semantic document processing workflow:
    1. Document loading with LangChain
    2. Document structure analysis for large documents
    3. Enhanced contextual retrieval with section-based contextualization
    4. Adaptive chunk sizing based on document characteristics
    5. Vision processing for images/charts/tables
    6. Semantic embeddings with OpenAI text-embedding-3-large
    7. Vector storage in Qdrant with enhanced metadata
    """

    def __init__(self, config: Optional[SemanticIngestionLargeDocumentConfig] = None):
        """
        Initialize the enhanced semantic ingestion pipeline.

        Args:
            config: Optional configuration object. If None, creates default config.
        """
        self.config = config or SemanticIngestionLargeDocumentConfig()
        self.logger = logging.getLogger(__name__)

        # Initialize the enhanced main document processor
        self.processor = EnhancedSemanticDocumentProcessor(self.config)

        self.logger.info(
            "🚀 Enhanced Semantic Ingestion Pipeline for Large Documents initialized"
        )
        ...
        self.logger.info(
            f"📚 Large Document Threshold: {self.config.long_document_threshold:,} chars"
        )
        self.logger.info(
            f"🧠 Enhanced Features: Structure Analysis={self.config.enable_document_structure_analysis}, Adaptive Sizing={self.config.adaptive_chunk_sizing}"
        )
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            "pipeline": {
                "name": "Enhanced Semantic Ingestion Pipeline for Large Documents",
                "version": "2.0.0",
                "features": [
                ...
                "large_document_features": {
                    ...
                    "enhanced_context_size": self.config.enhanced_context_size,
                    "document_summary_size": self.config.document_summary_size,
                },
            },
