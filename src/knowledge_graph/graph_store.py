...
class GraphNode:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {"id": self.id, "label": self.label, **self.properties}
...
class KnowledgeGraphStore:
    ...
    async def store_entities_and_relationships(
        ...
        try:
            with self.driver.session() as session:
                ...
                logger.info(
                    f"Stored {len(entities)} entities and {len(relationships)} relationships for {document_id}"
                )
                return True
    ...
    def _generate_entity_id(self, entity: Entity) -> str:
        ...
        return f"{entity.label}_{entity.normalized_text}".replace(" ", "_").replace(
            "-", "_"
        )
    ...
    def get_entity_relationships(
        self, entity_text: str, limit: int = 20
    ) -> List[Dict[str, Any]]:
    ...
    def delete_documents_by_paths(self, file_paths: List[str]) -> Dict[str, Any]:
        """Delete documents and their associated entities/relationships by file paths."""
        try:
            deleted_stats = {
                "documents_deleted": 0,
                "entities_deleted": 0,
            ...
            with self.driver.session() as session:
                for file_path in file_paths:
                    try:
                        # First, get statistics before deletion
                        stats_result = session.run(
                            """
                            MATCH (d:Document {id: $file_path})
                        ...
                        if stats:
                            deleted_stats["documents_deleted"] += stats["doc_count"]
                            deleted_stats["entities_deleted"] += stats["entity_count"]
                            deleted_stats["relationships_deleted"] += stats["rel_count"]
                        ...
                        # Delete entities mentioned only in this document
                        session.run(
                            """
                            MATCH (d:Document {id: $file_path})
                            MATCH (e:Entity)-[:MENTIONED_IN]->(d)
                        ...
                        # Remove MENTIONED_IN relationships for entities mentioned in other documents
                        session.run(
                            """
                            MATCH (d:Document {id: $file_path})
                            MATCH (e:Entity)-[m:MENTIONED_IN]->(d)
                            DELETE m
                            """,
                            file_path=file_path,
                        )
                        ...
                        logger.debug(
                            f"Deleted knowledge graph data for document: {file_path}"
                        )

                    except Exception as e:
                        error_msg = (
                            f"Error deleting {file_path} from knowledge graph: {e}"
                        )
                        logger.error(error_msg)
                        deleted_stats["errors"].append(error_msg)

            logger.info(f"Knowledge graph deletion completed: {deleted_stats}")
    ...
    def delete_documents_by_file_type(self, file_type: str) -> Dict[str, Any]:
        """Delete all documents of a specific file type and their associated data."""
        try:
            # First, find all documents of this file type
            with self.driver.session() as session:
                # Find document IDs that contain the file type in their metadata
                # Since we store document_id as file_path, we can't directly filter by file_type
                # We need to get all documents and filter by checking if the path ends with Excel extensions

                if file_type.lower() == "excel":
                    # For Excel files, look for .xlsx and .xls extensions
                    result = session.run(
                        """
                        MATCH (d:Document)
                        WHERE d.id ENDS WITH '.xlsx' OR d.id ENDS WITH '.xls'
                        RETURN d.id as file_path
                        """
                    )
                else:
                    # For other file types, you might need different logic
                    result = session.run(
                        """
                        MATCH (d:Document)
                        WHERE d.id CONTAINS $file_type
                        RETURN d.id as file_path
                        """,
                ...
                logger.info(
                    f"Found {len(file_paths)} documents with file type {file_type}"
                )

                # Use the existing method to delete by paths
                return self.delete_documents_by_paths(file_paths)

        except Exception as e:
            logger.error(f"Error deleting documents by file type {file_type}: {e}")
            return {
                "documents_deleted": 0,
                "entities_deleted": 0,
                "relationships_deleted": 0,
