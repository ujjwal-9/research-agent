...
def main():
    ...
    if results["success"]:
        ...
        if config.enable_parallel_processing:
            print(f"   - Max parallel documents: {config.max_parallel_documents}")
            print(f"   - Embedding batch size: {config.embedding_batch_size}")
        print(f"   - Semantic chunking: gradient method")
    ...
    print(
        f"   python -c \"from src.ingestion_semantic import SemanticIngestionPipeline; pipeline = SemanticIngestionPipeline(); print(pipeline.search('your query here'))\""
    )

    print(f"\n🎯 Semantic Features Used:")
    print(f"   ✅ Gradient-based semantic chunking for better boundaries")
    print(f"   ✅ Contextual retrieval for enhanced chunk semantics")
    print(f"   ✅ OpenAI text-embedding-3-large (3072 dimensions)")
    print(f"   ✅ Vision processing framework (images/charts/tables)")
    if config.enable_parallel_processing:
        print(f"   ✅ Parallel processing for faster document ingestion")

    print(f"\n⚡ Performance Tips:")
    if not config.enable_parallel_processing:
        print(
            f"   • Enable parallel processing: python scripts/run_semantic_ingestion.py --parallel"
        )
    print(
